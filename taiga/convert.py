...
class ConvertService(object):
    ...
    def hdf5_to_Rdata(self, hdf5_path, destination_file):
        ...
        stdout, stderr = handle.communicate(
            "require(rhdf5); fn <- %s; data <- t(h5read(fn,'/data')); dim.0 <- h5read(fn, '/dim_0'); dim.1 <- h5read(fn, '/dim_1'); dimnames(data) <- list(make.unique(dim.0), make.unique(dim.1)); data[is.nan(data)] <- NA ; save(data, file=%s)" % (
            r_escape_str(full_hdf5_path), r_escape_str(destination_file)))
    ...
    def Rdata_to_hdf5(self, input_path, dataset_id, hdf5_path, col_axis, row_axis):
        ...
        with tempfile.NamedTemporaryFile() as t:
            ...
            handle = subprocess.Popen(["R", "--vanilla"], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE)
            stdout, stderr = handle.communicate(
                "local.env <- new.env(); load(file=%s, local.env); write.table(local.env$data, file=%s, sep=',')" % (
                r_escape_str(input_path), r_escape_str(temp_path)))
            if handle.returncode != 0:
                raise Exception("R process failed: %s\n%s" % (stdout, stderr))
    ...
    def gct_to_hdf5(self, input_path, dataset_id, hdf5_path, col_axis, row_axis):
        ...
        assert data.shape == (len(names), len(col_header)), "Mismatch between data and dim names %r != %r" % (
        data.shape, (len(names), len(col_header)))
            ...
            for row in r:
                ...
                if len(data_row) == 0:
                    raise UserException(
                        ...
                        line, repr(row[0]), repr(delimiter)))
    ...
    def columnar_to_Rdata(self, input_file, destination_file):
        ...
        with tempfile.NamedTemporaryFile() as t:
            ...
            handle = subprocess.Popen(["R", "--vanilla"], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE)
            stdout, stderr = handle.communicate(
                "data <- read.table(%s, sep=',', head=T, as.is=T, check.names=F, quote='\"', comment.char=''); save(data, file=%s)" % (
                r_escape_str(temp_path), r_escape_str(destination_file)))
            if handle.returncode != 0:
                raise Exception("R process failed: %s\n%s" % (stdout, stderr))
