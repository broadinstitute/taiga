...
def get_dataset_by_name(meta_store, import_service, hdf5_store, cache_service):
    ...
    if "md5" in request.values:
        md5 = request.values["md5"]
        dataset_id = meta_store.find_by_md5(name, md5)
    else:
...
def get_metadata(meta_store, dataset_id):
    ...
    return jsonify(name=meta.name, created_timestamp=meta.created_timestamp, description=meta.description,
                   created_by=meta.created_by, version=meta.version, is_published=meta.is_published,
...
def _get_dataset(meta_store, import_service, hdf5_store, cache_service, dataset_id, parameters):
    ...
    if hdf5_path != None:
        if format == "tabular_csv":
            ...
            return flask.send_file(os.path.abspath(os.path.join(hdf5_store.hdf5_root, hdf5_path)), as_attachment=True,
                                   attachment_filename=generate_dataset_filename(meta_store, dataset_id, "hdf5"))
        else:
    ...
    return flask.send_file(file_handle.name, as_attachment=True,
                           attachment_filename=generate_dataset_filename(meta_store, dataset_id, suffix))
