...
def upload_columnar(import_service, meta_store):
    ...
    if not is_new_version_of_existing and versions_exist:
        return redirect_with_error("/upload/columnar-form",
                                   "There already exists a dataset with the name \"%s\"" % name)
    ...
    with NamedTemporaryFile() as temp_fd:
        ...
        meta_store.register_columnar_dataset(name, dataset_id, is_published,
                                             description, created_by_user_id, columnar_path, is_public,
                                             is_new_version_of_existing, permaname=permaname,
