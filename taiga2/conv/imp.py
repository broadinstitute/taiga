...
def gct_to_hdf5(progress, src_gct_file, dst_hdf5_file, rows_per_block=None,
...
def tcsv_to_hdf5(progress, src_csv_file, dst_hdf5_file, dialect, rows_per_block=None,
                 max_size_per_block=DEFAULT_MAX_ELEMENTS_PER_BLOCK, encoding="iso-8859-1"):
    ...
    with open(src_csv_file, 'rb') as tcsv:
        ...
        na_count, row_header = _convert_to_hdf5_file(row_count, col_count, col_header, dialect, dst_hdf5_file, progress,
                                                     r, rows_per_block, max_size_per_block, file=tcsv)
...
def _make_import_result(col_count, col_header, na_count, row_count, row_header, sha256):
    ...
    log.info("INFO: In _make_import_result, before processing. Memory is: {}GB".format(
    ...
    import_result = ImportResult(sha256=sha256,
                                 short_summary="{}x{} matrix, {} NAs".format(row_count, col_count, na_count),
                                 long_summary=long_summary)

    log.info("INFO: In _make_import_result, after processing. Memory is: {}GB".format(
        current_process.memory_info().rss / (1024 ** 2)))

    return import_result


def _convert_to_hdf5_file(row_count, col_count, col_header, dialect, dst_hdf5_file, progress, r, rows_per_block,
                          max_size_per_block, file, description_column=None):
    import h5py
    ...
    with h5py.File(dst_hdf5_file, 'w') as file_hdf5:
        ...
        for block_row_header, block_data in _read_rows_in_chunks(row_index + 1, progress, dst_hdf5_file, col_header, r,
                                                                 rows_per_block, dialect, file=file):
...
def _read_rows_in_chunks(first_line, progress, dst_hdf5_file, col_header, r, rows_per_chunk, dialect, file):
    row_header = []
    rows = []
    line = first_line
    ...
    for row in r:
        ...
        current_amount_MB_read = ((BYTES_PER_STR_OBJECT * len(data_row) * len(rows)) + (
                total_bytes_read - last_file_position)) / 1024 ** 2
