...
const CodeSampleSection = (props: Props) => {
  ...
  const rBlockLinks = s3AndVirtualDatafiles.map((df) => {
    const r_name = df.name.replace(/[^A-Za-z0-9]+/g, ".");
  ...
  const pythonBlockLines = s3AndVirtualDatafiles.map((df) => {
    const python_name = df.name.replace(/[^A-Za-z0-9]+/g, "_");
