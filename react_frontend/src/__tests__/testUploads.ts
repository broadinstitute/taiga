...
test("bad taiga id upload", () => {
  ...
  return t
    .upload(uploadFiles, params, callback)
    .then(() => {
      reachedBad[0] = true;
    })
    .catch(reason => {
      ...
      expect(lastStatus[0].progressMessage).toEqual(
        "Error: The following was not formatted like a valid taiga ID: invalid"
      );
    })
    .then(() => {
...
test(
  "end-to-end test of uploading with the tracker",
  () => {
    let uploadFile = "blob";
    ...
    let uploadFiles = [
      {
        name: "taiga",
        fileType: UploadFileType.TaigaPath,
        existingTaigaId: "sample-1.1/data"
      },
      {
        name: "upload",
