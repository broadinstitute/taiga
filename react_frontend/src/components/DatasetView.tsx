...
export interface DatasetViewProps
  extends RouteComponentProps<DatasetViewMatchParams> {
  tapi: TaigaApi;
  user: Models.User;
}
...
> {
  ...
  componentDidMount() {
    ...
    this.doFetch()
      .then(() => {
        ...
        // Update the url
        let last_index_dataset_permaname =
    ...
    return this.getTapi()
      ...
      .then(
        (
          ...
          if (
            (datasetAndDatasetVersion as Models.DatasetAndDatasetVersion)
              .dataset
          ) {
            ...
            datasetVersion = new Models.DatasetVersion(
              datasetAndDatasetVersion.datasetVersion,
              dataset
            );
            ...
            console.log(
              "Error in doFetch DatasetView. Type received is not a Dataset or a DatasetAndDatasetVersion"
            );
          }
        }
      )
  ...
  logAccess() {
    return this.getTapi()
      .create_or_update_entry_access_log(this.state.dataset.id)
      .then(() => {});
  }

  getActivityLog() {
    return this.getTapi()
      .get_activity_log_for_dataset_id(this.state.dataset.id)
  ...
  updateName(name: string) {
    this.getTapi()
      .update_dataset_name(this.state.datasetVersion.dataset_id, name)
      .then(() => {
        this.getActivityLog();
        return this.doFetch();
      });
    ...
    return this.getTapi()
      ...
      .then((result: DatafileUrl) => {
        if (!result.urls) {
          ...
          return this.delay(500).then(() => {
            ...
            return this.getOrLaunchConversion(
              datasetPermaname,
              version,
              datasetVersionId,
              datafileName,
      ...
      .catch((reason: any) => {
        ...
        this.setLoadingMessage(
          "Conversion error on server side: " +
            reason +
  ...
  forceExport() {
    ...
    this.getOrLaunchConversion(
      ...
      this.state.exportErrorInfo.conversionType,
      "Y"
    );
  }
  ...
  copyTo(alreadyFilledFolderId: string) {
    ...
    this.setState({
      ...
      callbackIntoFolderAction: (folderId: string) => {
        this.getTapi()
          .copy_to_folder([this.state.dataset.id], folderId)
  ...
  removeAccessLogs(arrayAccessLogs: Array<Models.AccessLog>): Promise<any> {
    return this.getTapi()
      .remove_entry_access_log(arrayAccessLogs)
      .then(() => {});
  }
  ...
  deprecateDatasetVersion(reason: string) {
    ...
    this.getTapi()
      .deprecate_dataset_version(this.state.datasetVersion.id, reason)
      .then(() => {
        this.doFetch();
        this.closeDeprecationReason();
      });
  ...
  deDeprecateDatasetVersion() {
    this.getTapi()
      .de_deprecate_dataset_version(this.state.datasetVersion.id)
      .then(() => {
        this.doFetch();
        this.closeDeprecationReason();
      });
  ...
  getCopyButton(datafile: Models.DatasetVersionDatafiles) {
    if (this.state.datasetVersion.state !== StatusEnum.Deleted) {
      ...
      let clipboard_content =
        dataset.permanames + "." + datasetVersion.version + "/" + datafile.name;
      ...
      return (
        <OverlayTrigger
          placement="left"
          trigger="click"
          overlay={tooltip_copied}
          rootClose={true}
        >
          <ClipboardButton
            data-clipboard-text={clipboard_content}
      ...
      return (
        <div>
          <div>
            ...
            <strong>
              {(row as DescriptionUpdateActivity).dataset_version}
            </strong>
          </div>
          {descriptionDetailsFormatter(
  ...
  renderChangesDescription() {
    ...
    return (
      <React.Fragment>
        ...
        {Dialogs.renderDescription(datasetVersion.changes_description)}
      </React.Fragment>
    );
  }
    ...
    if (!this.state) {
      ...
      if (dataset) {
        dataset.versions.sort(
          (datasetVersionA: NamedId, datasetVersionB: NamedId) => {
            return this.compareDatasetVersionsByVersionNumber(
              datasetVersionA,
              datasetVersionB
            );
          }
        );
        versions = dataset.versions.map(
          (dataset_version: NamedId, index: any) => {
            return (
              <span key={dataset_version.id}>
                {this.getLinkOrNot(dataset_version)}
                {dataset.versions.length !== index + 1 && <span>, </span>}
              </span>
            );
          }
        );
        ...
        folders = dataset.folders.map((f, index) => {
          return (
            <span key={index}>
              ...
              {dataset.folders.length !== index + 1 && <span>, </span>}
            </span>
          );
        });
      }
            ...
            if (df.underlying_file_id) {
              ...
              linkToUnderlying = (
                <div>
                  ...
                  (
                  <Link
                    to={relativePath("dataset/" + permaname + "/" + version)}
                  >
        ...
        leftNavsDialogs = (
          <span>
            ...
            <Dialogs.ShareEntries
              isVisible={this.state.showShareDatasetVersion}
              cancel={() => {
      ...
      return (
        <div>
          ...
          <Dialogs.ExportError
            isVisible={this.state.exportError}
